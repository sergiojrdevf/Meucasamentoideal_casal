	<section class="hidde space-default timer simple triangle overlay circle ">
		<div class="cntainer max_width ">
			<h2 class="a-center sans-serif color-theme section-title" id="title-time">Venha celebrar nosso amor com a gente em...</h2>
			<div class="column">
				<div class="xs-6-12 sm-3-12">
					<div class="timer-box a-center">
						<div class="number number-days">
						00
						</div>
						<div class="time">Dia(s)</div>
					</div>
				</div>
				<div class="xs-6-12 sm-3-12">
					<div class="timer-box a-center">
						<div class="number number-hours">
						00
						</div>
						<div class="time">Hora(s)</div>
					</div>
				</div>
				<div class="xs-6-12 sm-3-12">
					<div class="timer-box a-center">
						<div class="number number-minutes">
						00
						</div>
						<div class="time">Minuto(s)</div>
					</div>
				</div>
				<div class="xs-6-12 sm-3-12">
					<div class="timer-box a-center">
						<div class="number number-seconds">
						00
						</div>
						<div class="time">Segundo(s)</div>
					</div>
				</div>
			</div>
		</div>
	</section>